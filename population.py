...
class Population:
    ...
    def reproduce(self):
        ...
        for i in range(self.population_size):
            ...
            mutated_offspring_DNA = self.mutate(offspring_DNA)

            # Set weights with DNA.
            temp_individuals[i].brain.array_to_weights(mutated_offspring_DNA)
            # temp_individuals[i].brain.array_to_weights(offspring_DNA)

        # Set current individuals to new individuals.
        temp_individuals = temp_individuals.copy()[1:]
        temp_individuals.append(self.get_best_individual())
    ...
    def mutate(self, offspring_DNA):
        ...
        for i in range(len(offspring_DNA)):
            ...
            if(mutation_prob <= self.mutation_rate):
                mutated_DNA[i] = np.random.uniform(-0.5,0.5)
